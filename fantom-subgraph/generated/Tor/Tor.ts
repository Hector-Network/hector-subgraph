...
export class Tor extends ethereum.SmartContract {
  static bind(address: Address): Tor {
    return new Tor("Tor", address);
  }
  ...
  setMinter(minter_: Address): boolean {
    let result = super.call("setMinter", "setMinter(address):(bool)", [
  ...
  try_setMinter(minter_: Address): ethereum.CallResult<boolean> {
    let result = super.tryCall("setMinter", "setMinter(address):(bool)", [
      ethereum.Value.fromAddress(minter_)
    ]);
    if (result.reverted) {
      return new ethereum.CallResult();
